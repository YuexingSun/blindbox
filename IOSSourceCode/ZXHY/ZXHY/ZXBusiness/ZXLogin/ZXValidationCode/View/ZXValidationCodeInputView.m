//
//  ZXValidationCodeInputView.m
//  ZXHY
//
//  Created by Bern Mac on 8/10/21.
//

#import "ZXValidationCodeInputView.h"

@interface ZXValidationCodeInputView ()
<UITextFieldDelegate>
...
@implementation ZXValidationCodeInputView
...
- (void)configTextField{
    ...
    for (NSInteger i = 0; i < self.itemCount; i++){
        ...
        line.backgroundColor = WGRGBColor(180, 172, 188);
...
- (void)tfEditingChanged:(UITextField *)textField{
    ...
    //输入中
    if (self.delegate && [self.delegate respondsToSelector:@selector(inputValidationCodeingInputWithUITextField:CodeInputView:)]){
        [self.delegate inputValidationCodeingInputWithUITextField:textField CodeInputView:self];
    }
    ...
    if (textField.text.length >= self.itemCount) {
        ...
        if(self.delegate && [self.delegate respondsToSelector:@selector(didInputWithValidationCodeInputView:)]){
            [self.delegate didInputWithValidationCodeInputView:self];
        }
    }
}
