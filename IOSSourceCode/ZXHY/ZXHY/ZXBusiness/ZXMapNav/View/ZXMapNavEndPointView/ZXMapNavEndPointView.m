...
@interface ZXMapNavEndPointView()
...
@implementation ZXMapNavEndPointView
...
- (void)zx_initializationUI{
    ...
    UIImageView *unionImageView = [[UIImageView alloc] initWithFrame:CGRectZero];
...
- (void)zx_openResultslnglatModel:(ZXOpenResultsModel *)openResultsModel{
    ...
    NSString *imageStr = @"walkLogo";
    NSString *typeStr = @"步行";
    
    if (self.openResultsModel.currentNavType == ZXCurrentNavType_Walk){
        imageStr = @"walkLogo";
        typeStr = @"步行";
    }else{
        imageStr = @"driveLogo";
...
- (void)zx_updateNaviInfo:(AMapNaviInfo *)naviInfo{
    ...
    NSString *typeStr = (self.openResultsModel.currentNavType == 1) ? @"步行" : @"驾车";
