...
@interface ZXLocationTransFormTool ()


@end

@implementation ZXLocationTransFormTool
...
- (id)transformFromGPSToGD {
    CLLocationCoordinate2D coor = [ZXLocationTransFormTool transformFromWGSToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[ZXLocationTransFormTool alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromGDToBD {
    CLLocationCoordinate2D coor = [ZXLocationTransFormTool transformFromGCJToBaidu:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[ZXLocationTransFormTool alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromBDToGD {
    CLLocationCoordinate2D coor = [ZXLocationTransFormTool transformFromBaiduToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[ZXLocationTransFormTool alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromGDToGPS {
    CLLocationCoordinate2D coor = [ZXLocationTransFormTool transformFromGCJToWGS:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[ZXLocationTransFormTool alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromBDToGPS {
    //先把百度转化为高德
    CLLocationCoordinate2D start_coor = [ZXLocationTransFormTool transformFromBaiduToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    CLLocationCoordinate2D end_coor = [ZXLocationTransFormTool transformFromGCJToWGS:CLLocationCoordinate2DMake(start_coor.latitude, start_coor.longitude)];
    return [[ZXLocationTransFormTool alloc] initWithLatitude:end_coor.latitude andLongitude:end_coor.longitude];
}


- (id)transformFromGPSToBD{
    CLLocationCoordinate2D coor = [ZXLocationTransFormTool transformFromWGSToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    ZXLocationTransFormTool *locationTransFormTool = [[ZXLocationTransFormTool alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
    return [locationTransFormTool  transformFromGDToBD];
}
