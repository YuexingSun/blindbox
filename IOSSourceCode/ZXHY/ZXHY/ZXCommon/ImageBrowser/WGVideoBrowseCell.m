...
@interface NSObject (WGVideoPlayingRecord)
- (void)wg_videoPlayingAdd:(NSObject *)obj;
- (void)wg_videoPlayingRemove:(NSObject *)obj;
- (BOOL)wg_noVideoPlaying;
@end
@implementation NSObject (WGVideoPlayingRecord)
- (NSMutableSet *)wg_videoPlayingSet {
    static void *kWGRecordKey = &kWGRecordKey;
    NSMutableSet *set = objc_getAssociatedObject(self, kWGRecordKey);
    if (!set) {
        set = [NSMutableSet set];
        objc_setAssociatedObject(self, kWGRecordKey, set, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    return set;
}
- (void)wg_videoPlayingAdd:(NSObject *)obj {
    [[self wg_videoPlayingSet] addObject:[NSString stringWithFormat:@"%p", obj]];
}
- (void)wg_videoPlayingRemove:(NSObject *)obj {
    [[self wg_videoPlayingSet] removeObject:[NSString stringWithFormat:@"%p", obj]];
}
- (BOOL)wg_noVideoPlaying {
    return [self wg_videoPlayingSet].count == 0;
}
@end


@interface WGVideoBrowseCell()<UIGestureRecognizerDelegate, YBIBVideoDataDelegate,YBIBVideoViewDelegate> {
...
@implementation WGVideoBrowseCell

- (void)dealloc {
    [self.yb_backView wg_videoPlayingRemove:self];
}
...
- (void)prepareForReuse {
    ((WGVideoBrowseCellData *)self.yb_cellData).delegate = nil;
...
- (void)updateImageLayoutWithOrientation:(UIDeviceOrientation)orientation previousImageSize:(CGSize)previousImageSize {
    ...
    if (image) {
        [self.videoView updateActionBarYWithHeight];
    }
}
...
- (void)hideToolViews:(BOOL)hide {
    if (hide) {
        ...
        if ([self.yb_backView wg_noVideoPlaying]) {
...
- (void)yb_pageChanged {
    ...
    [self.videoView setPageTitle:titleStr];
    self.videoView.needAutoPlay = [[AFNetworkReachabilityManager sharedManager] isReachableViaWiFi];
...
- (void)yb_startPlayForVideoView:(YBIBVideoView *)view {
    ...
    [self.yb_backView wg_videoPlayingAdd:self];
    [self.yb_auxiliaryViewHandler() yb_hideLoadingWithContainer:self];
...
- (WGIBVideoView *)videoView {
    if (!_videoView) {
        _videoView = [WGIBVideoView new];
