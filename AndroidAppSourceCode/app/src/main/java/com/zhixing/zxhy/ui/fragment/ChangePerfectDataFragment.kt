 ...
 * 修改兴趣偏好页面
 */
class ChangePerfectDataFragment :
    BaseDbFragment<ChangePerfectDataViewModel, FragmentChangePerfectDataBinding>() {
                    ...
                    when(item.type) {
                        ...
                        2 -> {
                            item.itemlist?.forEach { itemlist ->
                                val chip = layoutInflater.inflate(R.layout.item_prefect_chip, ChipGroupA, false) as Chip
                                chip.apply {
                            ...
                            ChipGroupA.setOnCheckedChangeListener { group, itemId ->
                                run listRun@ {
                                    item.itemlist?.forEach {
                                        if (it?.itemname ?: "" == group.findViewById<Chip>(itemId).text.toString()) {
                                            questionList[position].ans = it?.itemid.toString()
                                            return@listRun
                                        }
                                    }
                                }
                        ...
                        //拖动条
                            ...
                            run listRun@ {
                                item.itemlist?.forEachIndexed { index, itemlist ->
                                    if (itemlist?.itemid ?: 0 == item.answer) {
                                        SliderA.value = index.toFloat()
                            ...
                            SliderA.addOnSliderTouchListener(object : Slider.OnSliderTouchListener {
                                ...
                                override fun onStopTrackingTouch(slider: Slider) {
                                    ...
                                    ItemName.text = item.itemlist?.get(slider.value.toInt())?.itemname ?: ""
                                }
                            })
                        }
                    }
