package org.flower.l.library.adapter
...
import org.flower.l.library.base.BaseAdapter
import org.flower.l.library.util.DensityExt.sp
...
class TabAdapter @JvmOverloads constructor(
    ...
    inner class DesignViewHolder : AppViewHolder(R.layout.item_tab_design) {
        ...
        override fun onBindView(position: Int) {
            ...
            titleView?.setTextColor(if (selectedPosition == position) selectedColor else unSelectedColor)
    ...
    inner class SlidingViewHolder : AppViewHolder(R.layout.item_tab_sliding),
        AnimatorUpdateListener {

        private val titleView: TextView? by lazy { findViewById(R.id.tv_tab_sliding_title) }
        private val lineView: View? by lazy { findViewById(R.id.shapeLine) }

        private val mDefaultTextSize: Int by lazy { 15.sp.toInt() }
        private val mSelectedTextSize: Int by lazy { 16.sp.toInt() }

        init {
            titleView?.textSize = mDefaultTextSize.toFloat()
            if (fixed) {
                val itemView: View = getItemView()
                val layoutParams: ViewGroup.LayoutParams = itemView.layoutParams
                layoutParams.width = ViewGroup.LayoutParams.MATCH_PARENT
        ...
        override fun onBindView(position: Int) {
            ...
            titleView?.let {
                ...
                it.setTextColor(if (selectedPosition == position) selectedColor else unSelectedColor)
                val textSize = it.textSize.toInt()
                if (selectedPosition == position) {
